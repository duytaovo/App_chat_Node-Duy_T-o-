  ...
  return new Promise((resolve, reject) => {
    ...
    const confirm = value => {
      ...
      if (innerParams.preConfirm) {
        ...
        const preConfirmPromise = Promise.resolve().then(() =>
          innerParams.preConfirm(value, innerParams.extraParams)
        )
        if (innerParams.expectRejections) {
          ...
          preConfirmPromise.then(
            preConfirmValue => {
              if (
                dom.isVisible(domCache.validationMessage) ||
    ...
    const keydownHandler = (e, innerParams) => {
      ...
      if (e.key === 'Enter' && !e.isComposing) {
        if (
          e.target &&
          this.getInput() &&
          e.target.outerHTML === this.getInput().outerHTML
        ) {
        ...
        if (
          ...
          dom.isVisible(domCache.cancelButton)
        ) {
          domCache.cancelButton.focus()
          // and vice versa
        } else if (
          document.activeElement === domCache.cancelButton &&
          dom.isVisible(domCache.confirmButton)
        ) {
    ...
    switch (innerParams.input) {
      ...
      case 'url': {
        ...
        if (
          ...
          warn(
            `Unexpected type of inputValue! Expected "string", "number" or "Promise", got "${typeof innerParams.inputValue}"`
          )
        }
      ...
      case 'checkbox': {
        const checkbox = dom.getChildByClass(
          domCache.content,
      ...
      case 'textarea': {
        const textarea = dom.getChildByClass(
          domCache.content,
      ...
      default:
        error(
          `Unexpected type of input! Expected "text", "email", "password", "number", "tel", "select", "radio", "checkbox", "textarea", "file" or "url", got "${innerParams.input}"`
        )
        break
    }
    ...
    if (innerParams.input === 'select' || innerParams.input === 'radio') {
      const processInputOptions = inputOptions =>
        populateInputOptions(formatInputOptions(inputOptions))
      if (isPromise(innerParams.inputOptions)) {
        ...
        error(
          `Unexpected type of inputOptions! Expected object, Map or Promise, got ${typeof innerParams.inputOptions}`
        )
      }
    } else if (
