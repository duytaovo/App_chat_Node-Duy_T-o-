function removeFriendFromGroup(id) {
  $(`ul#friends-added .remove-user[data-uid = ${id}]`)
    .off('click')
    .on('click', function () {
...
function addFriendsToGroup() {
  $('ul#group-chat-friends')
    ...
    .bind('click', function () {
      ...
      $('ul#group-chat-friends')
        .find('li[data-uid=' + uid + '] .contactPanel')
...
function callSearchFriend(e) {
  if (e.which === 13 || e.type === 'click') {
    ...
    if (!regKeyword.test(keyword)) {
      alertify.notify(
        'Nội dung tìm kiếm không được chứa kí tự đặc biệt, chỉ cho phép chữ cái, số và khoảng trống',
        'error',
        5
      )
      return
  ...
  $('#btn-create-group-chat')
    ...
    .on('click', function name() {
      ...
      if (countUsers.length < 2) {
        alertify.error(
          'Vui lòng chọn bạn bè để thêm vào nhóm, ít nhất thêm 2 người',
          5
        )
        return
      }
      ...
      if (
        !regUsername.test(groupChatName) ||
            ...
            data.groupChat.membersInfo.forEach(member => {
              ...
              <div id="${member._id}" class="col-sm-2 member">
                <div class="thumbnail">
                  ...
                  <div class="caption">
                    <p class="member-name ${
                      data.groupChat.userId === member._id ? 'admin' : ''
                    }">
                      ${member.username}
                    </p>
              ...
              $(`#membersModal_${data.groupChat._id}`)
...
$(document).ready(function () {
  ...
  socket.on('response-new-group-created', response => {
    ...
    response.groupChat.membersInfo.forEach(member => {
      let html = `
        <div id="${member._id}" class="col-sm-2 member">
          <div class="thumbnail">
            ...
            <div class="caption">
              <p class="member-name ${
                response.groupChat.userId === member._id ? 'admin' : ''
              }">
                ${member.username}
              </p>
              <div class="member-talk" data-uid="${member._id}" >
      ...
      $(`#membersModal_${response.groupChat._id}`)
        .find('.all-members .row')
        .append(html)
    })
