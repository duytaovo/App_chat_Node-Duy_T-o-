$(document).ready(function () {
  $('#link-read-more-all-chat').bind('click', function () {
    ...
    $.get(
      `/message/read-more-all-chat?skipPersonal=${skipPersonal}&skipGroup=${skipGroup}&personalIds[]=${personalIds}&groupIds[]=${groupIds}`,
      function (data) {
        if (data.leftSideData.trim() === '') {
          alertify.notify(
            'Bạn không còn cuộc trò chuyện nào để xem nữa.',
            'error',
            5
          )
        ...
        if (
          $(`#all-chat`).find('ul a.room-chat').length ===
          data.countAllConversations
        ) {
