...
function notSupportVideoChat() {
  $('.video-chat-group').on('click', function () {
    alertify.warning(
      'Tính năng này không khả dụng với nhóm trò truyện. Vui lòng thử lại tính năng này với trò chuyện cá nhân',
      5
    )
  })
}
    ...
    getUserMedia(
      ...
      function (err) {
        if (err.toString() === 'NotAllowedError: Permission denied') {
          alertify.error(
            'Xin lỗi, bạn đã tắt quyền truy cập vào thiết bị nghe gọi trên trình duyệt, vui lòng mở lại trong phần cài đặt của trình duyệt',
            5
          )
        }
        ...
        if (err.toString() === 'NotFoundError: Requested device not found') {
          alertify.error(
            'Xin lỗi, không tìm thấy thiết bị nghe gọi trên máy tính của bạn',
            5
          )
        }
      }
