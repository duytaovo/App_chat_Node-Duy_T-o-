function leaveGroupChat() {
  $('.leave-group-chat')
    ...
    .on('click', function (e) {
      ...
      let groupChatName = $(`.right[data-chat = ${targetId}]`)
        .find('.top .name')
        .val()
...
socket.on('response-leave-group-chat', response => {
  $('#screen-chat')
    .find(
      `.number-members[href = "#membersModal_${response.groupChat._id}"] .show-number-members`
    )
    .text(response.groupChat.usersAmount)

  $(`#membersModal_${response.groupChat._id}`)
    .find(`.member[id = ${response.currentUserId}]`)
    .remove()
})
