      ...
      if (elem.which === 13) {
        ...
        $.post(
          ...
          function (data) {
            ...
            let messageOfMe =
              $(`<div class="bubble me" data-mess-id="${data.newMessage._id}">
            ...
            $(`.person[data-chat = ${divId}]`)
              .find('span.preview')
              .html(emojione.toImage(data.newMessage.text))
...
$(document).ready(function () {
  socket.on('response-chat-text-emoji', function (response) {
    ...
    // step 01: handle message data before show
    let messageOfYou = $(`<div class="bubble ${
      response.newMessage.senderId == currentUserId ? 'me' : 'you'
    }" data-mess-id="${response.newMessage._id}">
                              <div class="bubble-content"></div>
                            </div>`)
    ...
    if ($('#all-chat').find(`li[data-chat = "${divId}"]`).length) {
      ...
      $(`.person[data-chat = ${divId}]`)
        .find('span.preview')
        .html(emojione.toImage(response.newMessage.text))
