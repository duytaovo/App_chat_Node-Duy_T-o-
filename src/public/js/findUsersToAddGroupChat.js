function addMemberToGroup(groupId) {
  $('.btn-add-member')
    ...
    .on('click', function (e) {
      ...
      let memberName = $(`#addMemberModal_${groupId}`)
        .find(`.member-item[data-uid = ${memberId}] .member-name p`)
        .text()
      let memberAvatar = $(`#addMemberModal_${groupId}`)
        .find(`.member-item[data-uid = ${memberId}] .member-avatar img`)
        .attr('src')
      ...
      Swal.fire({
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            $(`#addMemberModal_${data._id}`)
              .find(`.member-item[data-uid = ${member.id}]`)
              .remove()
          ...
          `
            $(`#membersModal_${data._id}`)
              .find(`.all-members .row`)
              .append(memberHtml)

            $(`#screen-chat`)
              .find(`.right[data-chat = ${data._id}] .show-number-members`)
              .text(data.usersAmount)
            talkWithMember()
...
function findUsersToAddGroupChat(e, divId) {
  if (e.which === 13 || e.type === 'click') {
    ...
    if (
      keyword.length < 1 ||
      keyword.length > 16 ||
      !regKeyword.test(keyword)
    ) {
      alertify.error(
        'Nội dung tìm kiếm không hợp lệ, giới hạn từ 1 - 16 kí tự và không chứa kí tự đặc biệt.',
        5
      )
      return
    }
    ...
    $.get(
      `/group-chat/find-user-to-add-group-chat?groupId=${divId}&keyword=${keyword}`,
      function (data) {
        if (!data.trim()) {
...
function bindEventfindUsersToAddGroupChat(divId) {
  $('.input-find-member')
    .off('keypress')
    .on('keypress', e => findUsersToAddGroupChat(e, divId))
  $('.btn-find-member')
    .off('click')
    .on('click', e => findUsersToAddGroupChat(e, divId))
}

socket.on('response-add-member-to-group-chat', response => {
  $(`#addMemberModal_${response.group._id}`)
    .find(`.member-item[data-uid = ${response.member.id}]`)
    .remove()
  ...
  let memberHtml = `
            <div id="${response.member.id}" class="col-sm-2 member">
              <div class="thumbnail">
                <img class="member-avatar" src="${
                  response.member.avatar
                }" alt="">
                <div class="caption">
                  <p class="member-name ${
                    response.member.id === response.group.userId ? 'admin' : ''
                  }">${response.member.name}</p>
          ...
          `
  $(`#membersModal_${response.group._id}`)
    .find(`.all-members .row`)
    .append(memberHtml)

  $(`#screen-chat`)
    .find(`.right[data-chat = ${response.group._id}] .show-number-members`)
    .text(response.group.usersAmount)
  talkWithMember()
})
...
socket.on('response-add-group-chat-for-new-member', response => {
  ...
  if (lastMess.messageType === 'text' || !lastMess.length) {
    leftSideData = `
        <a href="#uid_${
          response.group._id
        }" class="room-chat" data-target="#to_${response.group._id}">
  ...
  response.group.messages.forEach(message => {
    ...
    if (message.messageType === 'text') {
      messageHtml = `
      <div 
        class="convert-emoji bubble ${
          message.senderId == currentUserId ? 'me' : 'you'
        }"
        data-mess-id="${message._id}"
      >
        <img src="/images/users/${
          message.sender.avatar
        }" class="avatar-small" title="">
    ...
    if (message.messageType === 'image') {
      ...
      >
        ...
        <img
          src="data:${message.file.contentType}; base64, ${bufferToBase64(
        message.file.data.data
      )}"
    ...
    if (message.messageType === 'file') {
      ...
      >
        ...
        <div class="bubble-content">
          <a
            href="data:${message.file.contentType}; base64, ${bufferToBase64(
        message.file.data.data
      )}"
  ...
  response.group.messages.forEach(message => {
    if (message.messageType === 'image') {
      let messageHtml = `
        <img src="data:${message.file.contentType}; base64, ${bufferToBase64(
        message.file.data.data
      )}" />
      `

      $(`#imagesModal_${response.group._id}`)
        .find('.all-images')
        .append(messageHtml)
    }
  })
  ...
  response.group.messages.forEach(message => {
    if (message.messageType === 'file') {
      ...
      $(`#attachmentsModal_${response.group._id}`)
  ...
  response.group.membersInfo.forEach(member => {
    let html = `
      <div id="${member._id}" class="col-sm-2 member">
        <div class="thumbnail">
          <img class="member-avatar" src="/images/users/${
            member.avatar
          }" alt="">
          <div class="caption">
            <p class="member-name ${
              response.group.userId === member._id ? 'admin' : ''
            }">
              ${member.username}
            </p>
