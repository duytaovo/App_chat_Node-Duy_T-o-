    ...
    $.get(
      ...
      function (data) {
        if (!data.newContactUsers.length) {
          alertify.notify(
            'Bạn không còn yêu cầu nào để xem nữa cả.',
            'error',
            5
          )
        ...
        if (
          $(`#request-contact-received .contactList`).find('._contactList')
            .length === data.countAllContactsReceived
        ) {
