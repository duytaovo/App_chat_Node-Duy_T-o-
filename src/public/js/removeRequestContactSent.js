...
socket.on('response-remove-request-contact-sent', function (user) {
  $('.noti_content')
    .find(`div.notif-readed-false[data-uid = ${user.id}]`)
    .remove() // Xóa ở popup notification
  $('ul.list-notifications')
    .find(`li>div.notif-readed-false[data-uid = ${user.id}]`)
    .parent()
    .remove() // Xóa ở modal notification
