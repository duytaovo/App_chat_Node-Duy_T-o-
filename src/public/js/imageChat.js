    ...
    .on('change', function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          let messageOfMe = $(
            `<div class="bubble me bubble-image-file" data-mess-id="${data.newMessage._id}"></div>`
          )
          let imageChat = `<img 
                          src="data:${
                            data.newMessage.file.contentType
                          }; base64, ${bufferToBase64(
            data.newMessage.file.data.data
          ...
          $(`.person[data-chat = ${divId}]`)
            ...
            .html(
              moment(data.newMessage.createdAt)
                .locale('vi')
                .startOf('seconds')
                .fromNow()
            )
          ...
          $(`.person[data-chat = ${divId}]`).on(
            'event.moveConversationToTheTop',
            function () {
          ...
          let imageChatToModal = `<img src="data:${
            data.newMessage.file.contentType
          }; base64, ${bufferToBase64(data.newMessage.file.data.data)}"/>`
          $(`#imagesModal_${divId}`)
            .find('.all-images')
            .append(imageChatToModal)
        },
...
$(document).ready(function () {
  socket.on('response-chat-image', function (response) {
    ...
    let messageOfYou = $(
      `<div class="bubble ${
        response.newMessage.senderId == currentUserId ? 'me' : 'you'
      } bubble-image-file" data-mess-id="${response.newMessage._id}"></div>`
    )
    let imageChat = `<img 
                      src="data:${
                        response.newMessage.file.contentType
                      }; base64, ${bufferToBase64(
      response.newMessage.file.data.data
    )}" class="show-image-chat"
                    />`
    ...
    if ($('#all-chat').find(`li[data-chat = "${divId}"]`).length) {
      ...
      $(`.person[data-chat = ${divId}]`)
        .find('span.time')
        .html(
          moment(response.newMessage.createdAt)
            .locale('vi')
            .startOf('seconds')
            .fromNow()
        )
      ...
      let imageChatToModal = `<img src="data:${
        response.newMessage.file.contentType
      }; base64, ${bufferToBase64(response.newMessage.file.data.data)}"/>`
      ...
      leftSideData = `
          <a href="#uid_${divId}" class="room-chat" data-target="#to_${divId}">
            <li class="person group-chat" data-chat="${divId}">
              ...
              <span class="time ${
                response.newMessage.senderId != currentUserId &&
    ...
    response.getChatGroupReceiver.membersInfo.forEach(member => {
      let html = `
        <div class="col-sm-2">
          <div class="thumbnail">
            ...
            <div class="caption">
              <p class="member-name ${
                response.getChatGroupReceiver.userId === member._id
                  ? 'admin'
                  : ''
              }">
                ${member.username}
      ...
      $(`#membersModal_${response.getChatGroupReceiver._id}`)
