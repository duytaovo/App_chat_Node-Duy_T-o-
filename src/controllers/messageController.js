...
const addNewTextEmoji = async (req, res) => {
  ...
  try {
    ...
    let data = await message.addNewTextEmoji(
      sender,
      receiverId,
      messageVal,
      isChatGroup
...
const addNewImage = (req, res) => {
  imgMessageUploadFile(req, res, async error => {
    ...
    try {
      ...
      let data = await message.addNewImage(
        sender,
        receiverId,
        messageVal,
        isChatGroup
      ...
      await fsExtra.remove(
        `${app.image_message_directory}/${data.newMessage.file.fileName}`
      )
...
const addNewAttachment = (req, res) => {
  attachmentMessageUploadFile(req, res, async error => {
    ...
    try {
      ...
      let data = await message.addNewAttachment(
        sender,
        receiverId,
        messageVal,
        isChatGroup
      ...
      await fsExtra.remove(
        `${app.attachment_message_directory}/${data.newMessage.file.fileName}`
      )
...
const readMoreAllChat = async (req, res) => {
  try {
    ...
    let membersModalData = await renderFile(
      'src/views/main/readMoreConversations/_membersModal.ejs',
      dataToRender
    )
    let addMemberModalData = await renderFile(
      'src/views/main/readMoreConversations/_addMemberModal.ejs',
      dataToRender
    )
